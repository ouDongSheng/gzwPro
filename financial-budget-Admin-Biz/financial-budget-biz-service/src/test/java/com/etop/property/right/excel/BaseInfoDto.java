/*
package com.etop.property.right.excel;


import lombok.Data;

@Data
public class BaseInfoDto {
    private String companyName;
    private String shehuixinyongdaima;
    private String zhuzhijigoudaima;
    private String chanquandanwei;
    private String chanquandanweitongyishehui;
    private String chanquandanweizhuzijigoudaima;
    private String chuantoubili;
    private String zhijiegubi;
    private String chanquancengji;
    private String shifoukonggu;
    private String zhucehao;
    private String gongsiType;
    private String chengliriqi;
    private String hezunriqi;
    private String jingyingState;
    private String fadingdaibiaoren;
    private String zucezhiben;
    private String sheng;
    private String shengdaima;
    private String shi;
    private String shidama;
    private String suozaiquyu;
    private String quyudaima;
    private String qiyedizhi;
    private String yijiname;
    private String yijicode;
    private String erjiname;
    private String erjicode;
    private String jingyingfanwei;

}
*/
